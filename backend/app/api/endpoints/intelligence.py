...
def ingest_schema(payload: SchemaUploadRequest, db: Session = Depends(get_db)):
...
def trigger_demo_scenario(db: Session = Depends(get_db)):
    ...
    # The second ingest triggers the impact analysis and versioning
    intel_service.ingest_schema(db, "Property System", v2_schema)
    
    return {"message": "Demo scenario triggered successfully."}
